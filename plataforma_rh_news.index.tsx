...
// Importações corrigidas para evitar dupla inicialização
...
// Inicialização segura: se já existir uma instância ativa, reutiliza ela
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubNews = onSnapshot(newsRef, (snapshot) => {
      ...
      setLoading(false); // Dados recebidos, desliga loading
    }, (err) => {
...
// ==========================================
// PAINEL DE AVALIAÇÕES (MODERADOR) - CORRIGIDO
// ==========================================
function EvaluationsPanel({ newsletters, evaluations }) {
  // Correção: Inicializa dinamicamente e atualiza quando a lista carregar por completo
  const [selectedNewsId, setSelectedNewsId] = useState('');

  useEffect(() => {
    if (!selectedNewsId && newsletters.length > 0) {
      setSelectedNewsId(newsletters[0].id);
    }
  }, [newsletters, selectedNewsId]);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-[#0a0a0a]">
        <div className="text-center p-10 bg-[#111] rounded-2xl shadow-xl max-w-md w-full border border-gray-800">
          ...
          <p className="text-gray-500">Nenhuma edição publicada disponível no momento.</p>
        </div>
      </div>
    );
  }
